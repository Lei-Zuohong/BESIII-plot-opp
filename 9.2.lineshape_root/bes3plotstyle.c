...
void FormatData(TH1 *datahist, int color, int style)
{
  datahist->SetMarkerStyle(style);
...
void FormatData(TGraph *datahist, int color, int style)
{
  datahist->SetMarkerStyle(style);
  datahist->SetMarkerSize(1.2);
  datahist->SetMarkerColor(color);
...
void NameAxes(TGraph *datagraph, const char *xname, const char *yname)
{
  if (xname)
    datagraph->GetXaxis()->SetTitle(xname);
  if (yname)
    datagraph->GetYaxis()->SetTitle(yname);
}
