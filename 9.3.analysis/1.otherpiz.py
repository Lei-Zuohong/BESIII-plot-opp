...
def plot(energy=0,
    ...
    hstyle.set_axis(hist['r0'], r'M_{#gamma#gamma}', ytitle)
    hstyle.set_height(hist['r0'], 1.2)
    hstyle.set_marker(hist['r0'])
    hstyle.set_marker(hist['r1'], Markerstyle=22)
    hstyle.set_background(hist['m1'])
    hstyle.set_background(hist['m0'], Fillcolor=2, Linecolor=2)
    #for nameh in tfile:
    #    hist[nameh].GetYaxis().SetRangeUser(2, 4000)
    # canvas.SetLogy()
    # 2.get legend & arrow
    drawlist = []
...
if(float(sys.argv[1]) in energy_list):
    plot(float(sys.argv[1]),
         tree='fit4c',
         pictures=['9.1.analysis/1.otherpiz_cut.pdf', '9.1.analysis/1.otherpiz_cut.jpg'],
         stop='yes')
else:
